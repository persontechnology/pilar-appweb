...
class ValidadorEc
{
        ...
        // validarRucPersonaNatural('123456789')
        // validarRucSociedadPublica('123456789')
        // validarRucSociedadPrivada('123456789')
    ...
    public function validarIdentificacion($numero)
    {
        switch (true) {
            case $this->validarCedula($numero):
            case $this->validarRucPersonaNatural($numero):
            case $this->validarRucSociedadPublica($numero):
            case $this->validarRucSociedadPrivada($numero):
                return true;
            default:
                return false;
        }
     ...
     *
     * @throws \Throwable Cuando valor esta vacio, cuando no es dígito y
    ...
    {
        if (empty($numero)) {
            throw new \Throwable('Valor no puede estar vacio');
        }

        if (!ctype_digit($numero)) {
            throw new \Throwable('Valor ingresado solo puede tener dígitos');
        }

        if (strlen($numero) != $caracteres) {
            throw new \Throwable('Valor ingresado debe tener '.$caracteres.' caracteres');
        }
     ...
     *
     * @throws \Throwable Cuando el código de provincia no esta entre 00 y 24
     *
     * @return bool
     */
    protected function validarCodigoProvincia($numero)
    {
        if ($numero < 0 || $numero > 24) {
            throw new \Throwable('Codigo de Provincia (dos primeros dígitos) no deben ser mayor a 24 ni menores a 0');
        }
     ...
     *
     * @throws \Throwable Cuando el tercer digito no es válido. El mensaje
        ...
        switch ($tipo) {
            ...
            case 'ruc_natural':
                if ($numero < 0 || $numero > 5) {
                    throw new \Throwable('Tercer dígito debe ser mayor o igual a 0 y menor a 6 para cédulas y RUC de persona natural');
                }
                break;
            case 'ruc_privada':
                if ($numero != 9) {
                    throw new \Throwable('Tercer dígito debe ser igual a 9 para sociedades privadas');
                }
                break;

            case 'ruc_publica':
                if ($numero != 6) {
                    throw new \Throwable('Tercer dígito debe ser igual a 6 para sociedades públicas');
                }
                break;
            default:
                throw new \Throwable('Tipo de Identificacion no existe.');
                break;
        }
     ...
     * @param string $numero tercer dígito de CI/RUC
     *
     * @throws \Throwable Cuando el establecimiento es menor a 1
     *
     * @return bool
     */
    ...
    {
        if ($numero < 1) {
            throw new \Throwable('Código de establecimiento no puede ser 0');
        }
     ...
     *
     * @throws \Throwable Cuando los digitosIniciales no concuerdan contra
    ...
    protected function algoritmoModulo10($digitosIniciales, $digitoVerificador)
    {
        ...
        if ($resultado != $digitoVerificador) {
            throw new \Throwable('Dígitos iniciales no validan contra Dígito Idenficador');
        }
