    ...
    var calculateAmortization = function( elem, options, output_elem ){
        ...
        if ( loan_info!==0 ) {
            ...
            var output_content = '<table class="accrue-amortization table table-bordered">'+
                    ...
                    '<th class="accrue-payment-amount">Pago mensual</th>'+
                ...
                counter_interest = 0,
                counter_payment = 0,
                counter_balance = parseFloat(loan_info.original_amount);
            ...
            for ( var i=0; i<loan_info.num_payments; i++) { 
                ...
                var monto_f=  Math.round(loan_info.payment_amount_formatted *100)/100;
                ...
                var fecha_fila='<input type="hidden" value="'+fecha_pago.format('YYYY-MM-DD')+'" name="fecha_pago_tabla['+(i+1)+']" />';
                var pago_mensual_fila='<input type="hidden" value="'+monto_f+'" name="pago_mensual_tabla['+(i+1)+']" />';
                var interes_fila='<input type="hidden" value="'+interest_f+'" name="interes_tabla['+(i+1)+']"/>';
                var total_de_pago_fila='<input type="hidden" value="'+pagos_f+'" name="total_de_pago_tabla['+(i+1)+']"/>';
                var saldo_capital_fila='<input type="hidden" value="'+balance_f+'" name="saldo_capital_tabla['+(i+1)+']"/>';
                var total_pago_fila='<input type="hidden" value="'+total_f+'" name="total_pago_tabla['+(i+1)+']"/>';
                // Append a row to the table
                output_content = output_content+ 
                    '<tr>'+
                    '<'+cell_tag+' class="accrue-payment-number">'+numero_pago_fila+(i+1)+'</'+cell_tag+'>'+
                    '<'+cell_tag+' class="accrue-fecha-pago">'+fecha_fila+fecha_pago.format('YYYY-MM-DD')+'</'+cell_tag+'>'+
                    '<'+cell_tag+' class="accrue-payment-amount">'+pago_mensual_fila+formatNumber( monto_f, options )+'</'+cell_tag+'>'+
                    '<'+cell_tag+' class="accrue-total-interest">'+interes_fila+formatNumber( interest_f, options )+'</'+cell_tag+'>'+
                    '<'+cell_tag+' class="accrue-total-payments">'+total_de_pago_fila+formatNumber( pagos_f, options )+'</'+cell_tag+'>'+
                    '<'+cell_tag+' class="accrue-balance">'+saldo_capital_fila+formatNumber( balance_f, options )+'</'+cell_tag+'>'+
                    '<'+cell_tag+' class="accrue-total-suma">'+total_pago_fila+formatNumber( total_f, options )+'</'+cell_tag+'>'+
                    '</tr>';
            }
